...
def gather_statistics(base_dir: Path):
    ...
    for sidecar_path in tqdm(sidecar_paths, desc="Processing trees"):
        # Load sidecar JSON directly to bypass Hydra config requirements
        with open(sidecar_path, 'r') as f:
            sidecar_data = json.load(f)
            
        # The sidecar stores the path to its corresponding tree
        tree_path = Path(sidecar_data["tree_path"])
        if not tree_path.exists():
            # Fallback to looking in the same directory if absolute path breaks
        ...
        # Extract scores
        inf_scores = sidecar_data.get("inference_scores", {})
        qp_costs = sidecar_data.get("question_presence_costs", {})
        ent_costs = sidecar_data.get("entailment_costs", {})
        
        for node_idx, depth in depths.items():
            str_idx = str(node_idx) # JSON keys are strings
            node = tree.get_node(node_idx)
            
            if node.node_type == NodeType.INFERENCE:
                if str_idx in inf_scores:
                    inference_records.append({
                        "Tree_ID": tree_id,
                        "Node_ID": node_idx,
                        "Depth": depth,
                        "Score": inf_scores[str_idx]
                    })
                    
            elif node.node_type == NodeType.CLARIFICATION_QUESTION:
                if str_idx in qp_costs:
                    qp_records.append({
                        "Tree_ID": tree_id,
                        "Node_ID": node_idx,
                        "Depth": depth,
                        "Score": qp_costs[str_idx]
                    })
                if str_idx in ent_costs:
                    entailment_records.append({
                        "Tree_ID": tree_id,
                        "Node_ID": node_idx,
                        "Depth": depth,
                        "Score": ent_costs[str_idx]
                    })
...
if __name__ == "__main__":
    ...
    data_directory = Path("./data/trees") 
    output_directory = Path("./data/eval_visualizations")
