...
if TYPE_CHECKING:
    from clarification_trees.dialog_tree import DialogTrajectory, DialogTree, TreeSidecar
...
class TransformersModelV2:
    ...
    device: str
    max_new_tokens: int
    image_resize_config: DictConfig | None
    ...
    def construct_lora_adapter(self, lora_config: DictConfig, adapter_name: str) -> None:
        """
        ...
        container_config = OmegaConf.to_container(lora_config, resolve=True)
        lora_config_obj = LoraConfig(**container_config)
    ...
    def preprocess_generation_inputs(self, trajectory: "DialogTrajectory", role: Literal["user", "assistant"]):
    ...
    def preprocess_rl_training_inputs(self, parent_node_idx: int, tree: "DialogTree", sidecar: "TreeSidecar", role: Literal["user", "assistant"]) -> tuple[List["TransformersModelV2.RLTrainingDatapoint"], float, float]:
        ...
        prompt_image_grid = prompt_inputs["image_grid_thw"][0]
